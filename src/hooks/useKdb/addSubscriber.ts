...
import { Setters } from "./attachListener";
...
export const addSubscriber = (
  ...
  if (subscribers[url] === undefined) {
    ...
    webSocketInstance.onclose = (event: WebSocketEventMap["close"]) => {
      subscribers[url].forEach(subscriber => {
        subscriber.setReadyState(prev =>
          Object.assign({}, prev, { [url]: READY_STATE_CLOSED })
        );
    ...
    webSocketInstance.onopen = (event: WebSocketEventMap["open"]) => {
      subscribers[url].forEach(subscriber => {
        subscriber.setReadyState(prev =>
          Object.assign({}, prev, { [url]: READY_STATE_OPEN })
        );
  ...
  return () => {
    if (subscribers[url] !== undefined) {
      ...
      if (index === -1) {
        throw new Error(
          "A subscriber that is no longer registered has attempted to unsubscribe"
        );
      }
      if (subscribers[url].length === 1) {
        subscribers[url][0].setReadyState(prev =>
          Object.assign({}, prev, { [url]: READY_STATE_CLOSING })
        );
